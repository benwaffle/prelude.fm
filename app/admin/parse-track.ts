...
const classicalMetadataSchema = z.object({
  ...
  catalogSystem: z.string().nullable().describe("Catalog system: Op, RV, BWV, K, Kk, Hob, D, S, etc. Null if not classical or no catalog number. If the work has multiple catalog numbers, pick the most popular one (e.g. for Vivaldi, with Op and RV, pick RV. for Smetana, pick JB)"),
  ...
  movementName: z.string().nullable().describe("Movement name like 'Finale: Alla breve', 'Allegro', null if unknown. If it is a single-movement work, this can be the key of the piece"),
